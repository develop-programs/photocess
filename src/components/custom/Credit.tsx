...
async function fetchData(email: string) {
  const response = await axios.get(
    `http://localhost:3000/api/auth?email=${email}&subscription=true`
  );
  ...
  return (
    <Suspense fallback={<div></div>}>
      <div className="flex items-center gap-2">
        ...
        <span>
          {data.subscription.credit === 0 ? 0 : data.subscription.credit}/400
        </span>
      </div>
    </Suspense>
  );
